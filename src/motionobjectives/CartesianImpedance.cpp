...
void CartesianImpedance::setVelocity(RobotState &robotstate){
    ...
    double sample_time = ros::Time::now().toSec() - Frame_root_tip_previous_.second.toSec();
    KDL::Twist ee_vel = KDL::diff(Frame_root_tip_previous_.first , frame_root_tip, sample_time);

    ROS_INFO("Previous end-effector pose = %f %f %f", Frame_root_tip_previous_.first.p.x(), Frame_root_tip_previous_.first.p.y(), Frame_root_tip_previous_.first.p.z());
    ROS_INFO("Current  end-effector pose = %f %f %f", frame_root_tip.p.x(), frame_root_tip.p.y(), frame_root_tip.p.z());
    ROS_INFO("Refreshing the end-effector velocity = %f %f %f %f %f %f with sample time %f", ee_vel.vel.x(),ee_vel.vel.y(), ee_vel.vel.z(), ee_vel.rot.x(),ee_vel.rot.y(),ee_vel.rot.z(),sample_time);

    ref_generator_[0].setCurrentVelocity(ee_vel.vel.x());
    ref_generator_[1].setCurrentVelocity(ee_vel.vel.y());
    ref_generator_[2].setCurrentVelocity(ee_vel.vel.z());
    ref_generator_[3].setCurrentVelocity(ee_vel.rot.x());
    ref_generator_[4].setCurrentVelocity(ee_vel.rot.y());
    ref_generator_[5].setCurrentVelocity(ee_vel.rot.z());
...
void CartesianImpedance::apply(RobotState &robotstate) {
    ...
    Frame_root_tip_previous_.first = frame_root_tip;
    Frame_root_tip_previous_.second = ros::Time::now();
}
